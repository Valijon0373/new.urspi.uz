    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col gap-4">
        {mockPositions.map((pos) => (
          <div key={pos.id} className="flex flex-col sm:flex-row items-start sm:items-center justify-between p-5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-sm">
            <div>
              <h3 className="text-lg font-bold text-slate-800 dark:text-slate-100">{pos.title}</h3>
              <p className="text-sm text-slate-400 mt-1">ID: {pos.id}</p>
            </div>
